...
## Phase 9: フレーズ自動分割（長尺対応）

- **無音でのみ分割**（[phrases.py](server/pitch/phrases.py) / 10.4）: RMS < -50dB が 150ms 以上
  続く区間の中央で分割。**有声区間の途中では絶対に切らない**。
- **フレーズ単位で解析**: 各フレーズを独立に WORLD 解析（sp/ap のメモリ肥大を回避）。
  ノート/f0/rms はグローバル時間へ合成してクライアントへ返す（画面は1本の timeline）。
- **フレーズ単位の再合成キャッシュ**: 入力(ローカルノート+reverb)のハッシュで判定し、
  **変わったフレーズだけ**を再合成。長尺でも1操作が軽い。連結は元の位置へ加算配置
  （リバーブの尾は次フレーズ前の無音へ自然に伸び、無音境界なのでクリックは出ない・P2）。
- マスター/リミッターは連結後に全体へ1回だけ（プレビュー=書き出しの一致は維持・AC-16）。
- クライアント: フレーズ境界を薄いシアンの破線で表示。

> **テイク管理・レイテンシ補正**は本フェーズの範囲だが、レイテンシ補正は
> 「伴奏を聴きながらの録音」に固有で、現状のアカペラ録音（11章）では発生しないため未実装。
> テイク管理（複数テイクの保持・差し替え）は録音ワークフローの拡張として今後対応。

### Phase 9 検証

| 項目 | 検証 |
|------|------|
| 無音でのみ分割・有声は切らない | `detect_phrases` 単体（連続音=1・150ms未満=1・無音=2）✅ |
| フレーズ連結が時間軸を保つ | 各フレーズが正しい位置に配置・間は無音 ✅ |
| 1フレーズ編集で他はキャッシュ再利用 | 編集フレーズのみ再合成・他は同一オブジェクト ✅ |

```bash
python -m pytest tests/test_phase9.py -v
```

## Phase 10: 音質の作り込み（選択的再合成・空間系・ノイズ低減）

仕様の骨格が通ったあと、実際に歌を通して見つかった品質問題への対応。
いずれも「なぜそうするか」が音の理屈で説明できる形にしてある。

- **選択的再合成**（[app.py](server/app.py) `_render_phrase`）: WORLD を通すと僅かに質感が変わる。
  そこで**実際にピッチ編集した区間だけ**を合成音にし、他は原音 PCM を使う（Hann 平滑した
  ゲートでブレンド。ゲート端では両者の音高が一致するので繋ぎ目が出ない）。
  音量だけの編集ならボコーダーを通さず、無編集フレーズは原音をビット一致で返す。
- **ルックアヘッド・リミッター**（[render.py](server/pitch/render.py) `true_peak_limit`）:
  64 サンプルのブロックゲインに 6 ブロックの先読みと 0.5 dB/ブロックのリリース制限。
  旧実装（超過時に信号全体を一律スケール）は、マスターを上げても同じだけ下げられて
  **操作が打ち消されていた**。
- **リバーブの作り直し**（`make_reverb_ir`）: 指数減衰させた白色雑音をそのまま畳み込むと
  「箱の中」のように籠る。プリディレイ 22ms・不等間隔の初期反射 9 個・高域ほど速い減衰・
  残響側の低域カット（190Hz / 24dB/oct）を入れ、dry を減らさず wet を足すセンド方式にした。
- **ディレイ**（`apply_delay`）: Time / Feedback / Mix。短い遅延ほど遅延時間をゆっくり
  揺らす（コムフィルタによる金属的な色付きを散らす）。効きは 40ms 以下で最大、120ms 以上で
  ゼロへ**連続的に**変化するので、値を変えても音が急変する境目が無い。
- **ノイズ低減**（`apply_noise_gate`）: 下方伸張。閾値はノイズフロア推定から自動、
  つまみは減衰量（dB）だけを変える。ゲイン曲線は 30ms 幅で平滑化してから掛ける。
- **includePrimary フラグ**: ハモリ声部だけを合成する経路。クライアントが声部ごとに
  独立した音声を持ち、再生中の ON/OFF を音量ノードで即座に切り替えるために使う。
  主ボイスを空配列にすると「無編集扱い」で原音が返るため、明示的なフラグが必要。

### Phase 10 検証（実測）

| 項目 | 実測 |
|------|------|
| 無編集フレーズは原音と一致 | maxdiff < 1e-6（ビット一致）✅ |
| 編集していない区間は原音のまま | 分割して片方だけ移動 → 未編集側 maxdiff < 1e-6 ✅ |
| ブレンド境界にクリックが出ない | 隣接サンプル差分比 ≈ 1.00 ✅ |
| 音量のみの編集はボコーダーを通らない | `orig × gainLinear` と厳密一致 ✅ |
| マスター +18dB が実際に効く | RMS +16.8dB（旧実装は打ち消されていた）✅ |
| 天井は常に守る | +6/+12/+24dB いずれも ≤ -1.0 dBTP ✅ |
| リバーブの籠り | 低域/中域バランス 旧 +7.5dB → 新 +6.0dB（原音 +6.7dB より澄む）✅ |
| リバーブの立ち上がり | 0.0ms → 22.4ms（直接音と分離）✅ |
| 高域の減衰 | 1.20s → 0.99s（中域 1.2s は維持）✅ |
| ディレイの間隔精度 | 300ms 設定で 300.0 / 600.1 / 900.2ms に繰り返し ✅ |
| ディレイの色付き | 揺らぎ無し 4.78dB → 有り 3.98dB（原音 4.00dB と同等）✅ |
| ノイズ低減の設定値と実測 | -6/-12/-18/-24/-40dB すべて誤差 ±0.0dB・声は ±0.00dB ✅ |
| ハモリ声部だけの合成 | 主トラックとの相関 -0.000（原音が漏れていない）✅ |
| 分けて合成した和 = まとめて合成 | 差 -148.8dB（実質完全一致）✅ |

```bash
python -m pytest tests/test_selective.py tests/test_limiter.py -v
```

## Phase 11: 自動ハモリ（キー判定と度数計算）

録音1（主トラック）からキーを判定し、キーに沿った度数でハモリ声部を作る。
判定と度数計算は **DOM 非依存の純粋関数**として [pitchlogic.js](server/static/pitchlogic.js) に置き、
ブラウザ無しでテストできるようにしてある。

- **キー判定** `detectKey`: 解析済みノートの `baseCents` を音価で重み付けした音高クラス分布を作り、
  Krumhansl-Schmuckler の 24 プロファイルと相関を取る。サーバー通信も再解析も不要。
- **チューニングずれの補正** `estimateTuningOffset`: 各音の「最寄り半音からのずれ」の
  音価重み付き円環平均。ただし**ずれの集中度が低い（＝一律ではなく散らばっている）ときは
  補正しない**。ここを見ないと、歌が不安定なだけの素材に大きな偽の補正が掛かって判定が壊れる
  （実測: ±45cent のばらつきで 47cent の誤補正が出て C major → E minor と誤判定）。
- **度数計算** `diatonicShift`: キー内の段数で数えるので「3度上」が長3度/短3度に自動で
  切り替わる（C→E は +4半音、E→G は +3半音）。音階外の音は最寄りの音階音へ寄せた上で
  半音のズレを保持する（ブルーノートを潰さない）。
- **確信度の表示**: 1位と2位の相関差。これが小さいか判定に使った音が 12 未満なら UI に
  警告を出す。相対調（平行調）の取り違えは原理的に消せないため、手動上書きを必ず用意する。

### Phase 11 検証（誤差を注入したシミュレーション・各600試行）

「音程がずれた録音でもキー判定が持つか」を実測した。

| ランダムなずれ σ | 判定成功率 | | 半音間違い | 成功率 |
|---|---|---|---|---|
| 0 cent | 60.2% | | 0音/20音 | 59.8% |
| 15 cent | 60.2% | | 2音/20音 | 57.3% |
| 25 cent | 58.3% | | 3音/20音 | 50.2% |
| 35 cent | 51.5% | | 5音/20音 | 47.0% |
| 50 cent | 37.3% | | | |

**25cent（半音の1/4）までは実質無傷。50cent は隣の半音との中間なので原理的に決められない。**
半音まるごとの間違いも 2音/20音までならほぼ影響しない（音価で重み付けしているため、
短い音の間違いは特に軽い）。

一律のずれ（チューニングずれ）は補正の有無で大きく変わる。
...
精度を支配するのは**ずれよりも音数**（σ=25cent・半音間違い1音の条件）。

| 音数 | 8 | 12 | 20 | 40 | 80 |
|---|---|---|---|---|---|
| 成功率 | 31.8% | 39.0% | 54.0% | 73.8% | **89.3%** |

> 上の絶対値はランダムに音階音を並べた合成旋律で測ったもので、実際の音楽より不利な条件。
> 実旋律には主音で終わる等の調性の手がかりがあり判定は容易になる（実データでは
> C major を相関 +0.857・2位との差 0.115 で明確に判定）。**相対的な劣化の読み取りに使う数字**。

```bash
node tests/test_pitchlogic.js     # キー判定・ずれ補正・度数計算・ガイド生成を含む
```

## 全体の受け入れ基準

**AC-1〜21 をすべて自動テストで通過**（`python -m pytest tests/` = 35、
`node tests/test_pitchlogic.js` + `test_reclogic.js` = 114 チェック）。
